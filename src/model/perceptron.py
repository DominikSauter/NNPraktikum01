...
__author__ = "Dominik Sauter"  # Adjust this when you copy the file
__email__ = "dominik.sauter@student.kit.edu"  # Adjust this when you copy the file
...
class Perceptron(Classifier):
    ...
    def train(self, verbose=True):
        ...
        # use the trainingSet epochs times to train the network
        for k in xrange(self.epochs):
            # for all instances (one instance = one image)
            for j in xrange(len(self.trainingSet.input)):
                # calculate output with fire-function
                output = self.fire(self.trainingSet.input[j])
                # update weights after each instance
                for i in xrange(len(self.trainingSet.input[j])):
                    self.weight[i] = self.weight[i] + self.learningRate * (self.trainingSet.label[j] - output) * self.trainingSet.input[j][i]
